...
def insert_value(img, main_dir, y, x):
    ...
    sita = get_dir(img, y0+1, x0)*(y-y0)*(x0+1-x) + get_dir(img, y0, x0)*(y0+1-y)*(x0+1-x) + \
        get_dir(img, y0, x0+1)*(y0+1-y)*(x-x0) + \
        get_dir(img, y0+1, x0+1)*(y-y0)*(x-x0) - main_dir
    while(sita < 0):
        sita += 2*pi
    return sita
...
def draw_matches(img1, img2, des1, des2, ratio):
    ...
    for i in range(len(des1)):
        ...
        if(minm/second_minm < ratio):
            ...
            cv2.circle(res, (int(np.round(x1)), int(
                np.round(y1))), 2, (a, b, c), 1)
            cv2.circle(res, (int(np.round(x2)+cols1),
                             int(np.round(y2))), 2, (a, b, c), 1)
            cv2.line(res, (int(np.round(x1)), int(np.round(y1))), (int(np.round(
                x2)+cols1), int(np.round(y2))), (a, b, c), 1, lineType=cv2.CV_AA, shift=0)
...
def main():
    ...
    draw_matches(img1, img2, des1, des2, 0.86)
